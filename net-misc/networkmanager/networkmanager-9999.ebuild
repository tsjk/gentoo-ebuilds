...
+introspection kernel_linux +modemmanager +ncurses +nss +openrc +ppp resolvconf \
selinux systemd teamd test vala +wext +wifi zeroconf" # wimax
KEYWORDS=""
REQUIRED_USE="
	modemmanager? ( ppp )
	^^ ( nss gnutls )
...
src_prepare() {
	DOC_CONTENTS="To modify system network connections without needing to enter the root password, add your user account to the 'plugdev' group."
	...
	sed -e 's:/var/run/:/run/:' -i configure.ac || die
...
src_install() {
	...
	default_src_install completiondir="$(get_bashcompdir)"
