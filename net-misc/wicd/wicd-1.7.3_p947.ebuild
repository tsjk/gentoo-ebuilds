...
SRC_URI="http://launchpad.net/wicd/1.7/${PV%_p[0-9]*}/+download/${P%_p[0-9]*}.tar.gz
...
src_prepare() {
	...
	# Catch up: revision 933 -> 947
	epatch "${FILESDIR}"/${P%_p[0-9]*}-933_947.patch
	...
	epatch -p1 "${FILESDIR}"/${P%_p[0-9]*}-add-missing-gnome-shell-extension.patch
	# If LANG is undefined, build can fail (bug 537202)
	epatch "${FILESDIR}"/${P%_p[0-9]*}-undefined-LANG.patch
