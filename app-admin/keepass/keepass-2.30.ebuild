...
src_prepare() {
	...
	find "${S}" -name "*.sln" -exec sed -i 's/Format Version 10.00/Format Version 11.00/g' {} \;
	find "${S}" -name "*.csproj" -exec sed -i 's#ToolsVersion="3.5"#ToolsVersion="4.0"#g; s#<TargetFrameworkVersion>.*</TargetFrameworkVersion>##g; s#<PropertyGroup>#<PropertyGroup><TargetFrameworkVersion>v4.5</TargetFrameworkVersion>#g' {} \;
}
