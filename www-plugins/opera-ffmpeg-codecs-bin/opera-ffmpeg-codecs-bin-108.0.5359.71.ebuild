...
SRC_URI="http://security.ubuntu.com/ubuntu/pool/universe/c/chromium-browser/chromium-codecs-ffmpeg-extra_108.0.5359.71-0ubuntu0.18.04.5_amd64.deb"
...
_update() {
 ( set -x; f=$(curl -qs 'http://security.ubuntu.com/ubuntu/pool/universe/c/chromium-browser/' | grep -oP '(?<=")chromium-codecs-ffmpeg-extra_[^"]*_amd64\.deb' | sort -V | tail -n 1) && \
   ...
   E=$(ls -1 *.ebuild | sort -V | head -n 1 | sed "s@-bin-.*@-bin-${v}.ebuild@") && { [[ "${e}" == "${E}" ]] || mv -v "${e}" "${E}"; } && ebuild "${E}" digest )
}
