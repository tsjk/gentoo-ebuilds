...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="+binary-pluginloader flash foxitpdf grandstream installation-dialogs shockwave +silverlight static unity3d widevine"
...
pkg_postinst() {
	...
	use grandstream && ln -sf /usr/$(get_libdir)/pipelight/libpipelight-grandstream.so /usr/$(get_libdir)/nsbrowser/plugins/libpipelight-grandstream.so
        ...
        use widevine && ln -sf /usr/$(get_libdir)/pipelight/libpipelight-widevine.so /usr/$(get_libdir)/nsbrowser/plugins/libpipelight-widevine.so
...
pkg_prerm() {
	...
	if [ -h /usr/$(get_libdir)/nsbrowser/plugins/libpipelight-widevine.so ] ; then
		rm /usr/$(get_libdir)/nsbrowser/plugins/libpipelight-widevine.so
	fi
}
