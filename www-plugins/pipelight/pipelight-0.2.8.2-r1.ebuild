...
SRC_URI="https://bitbucket.org/mmueller2012/pipelight/get/v${PV}.tar.gz -> ${P}.tar.gz
	binary-pluginloader? ( http://repos.fds-team.de/pluginloader/v0.2.8.2/pluginloader.tar.gz -> pluginloader-prebuilt-v0.2.8.2.tar.xz )"
...
RDEPEND="${DEPEND}
	app-arch/cabextract
	|| ( app-emulation/wine[X,abi_x86_32,pipelight] app-emulation/wine-staging[X,abi_x86_32] app-emulation/wine-compholio[X,abi_x86_32] )
	x11-apps/mesa-progs"
...
src_prepare() {
	...
	if use !binary-pluginloader ; then
		if ! has_version "cross-i686-w64-mingw32/gcc" || ! has_version "cross-x86_64-w64-mingw32/gcc"; then
			...
			eerror "Then run 'crossdev -t i686-w64-mingw32 && crossdev -t x86_64-w64-mingw32'"
			eerror
...
src_configure() {
	...
	if use binary-pluginloader; then
		...
		if use static; then
			myargs+=" --win32-static --win64-static"
		else
			myargs+=" --gcc-runtime-dlls=/usr/$(get_libdir)/gcc/i686-w64-mingw32/$(i686-w64-mingw32-gcc -v |& grep 'gcc version' | awk '{print $3}')"
			myargs+=" --win32-cxx=$(which i686-w64-mingw32-g++)"
			myargs+=" --with-win64"
			myargs+=" --win64-cxx=$(which x86_64-w64-mingw32-g++)"
			myargs+=" --wine64-path=$(which wine64)"
		fi
	fi
