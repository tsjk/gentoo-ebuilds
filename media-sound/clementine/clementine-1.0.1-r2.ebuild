...
KDE_LINGUAS="ar be bg bn br bs ca cs cy da de el en_CA en_GB eo es et eu fa fi fr gl he hi hr hu hy ia id is it ja ka kk ko lt lv mr ms nb nl oc pa pl pt_BR pt ro ru sk sl sr@latin sr sv tr uk vi zh_CN zh_TW"

inherit eutils kde4-base gnome2-utils virtualx
...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="ayatana cdda +dbus gio ios ipod kde lastfm mms mtp projectm spotify spotify-blob test +udev wiimote"

REQUIRED_USE="
	ios? ( ipod )
	spotify-blob? ( spotify )
	udev? ( dbus )
	wiimote? ( dbus )
"
...
src_prepare() {
	...
	epatch "${FILESDIR}"/${P}-fix_spotifyblob_install_prefix.patch
