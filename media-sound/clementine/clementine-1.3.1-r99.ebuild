...
[[ ${PV} == *9999* ]] || \
KEYWORDS="~amd64 ~x86"
IUSE="amazoncloud box cdda +dbus debug dropbox googledrive ipod lastfm mms moodbar mtp projectm pulseaudio skydrive test +udisks vkontakte wiimote gio spotify spotify-blob spotify-downloader"
IUSE+="${LANGS// / linguas_}"
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DENABLE_SPOTIFY_BLOB="$(usex spotify-blob)"
		-DENABLE_SPOTIFY_DOWNLOADER="$(usex spotify-downloader)"
